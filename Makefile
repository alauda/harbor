...
HTTPPROXY=
BUILDBIN=true
NPM_REGISTRY=https://build-nexus.alauda.cn/repository/npm/
...
PUSHBASEIMAGE=
BASEIMAGETAG=v2.2.1-${BUILD_NUMBER}
BASEIMAGENAMESPACE=goharbor
# for harbor package name
PKGVERSIONTAG=alauda-v2.2.1-${BUILD_NUMBER}-amd64
...
SWAGGER=$(DOCKERCMD) run --rm -u $(shell id -u):$(shell id -g) -v $(COMPILEBUILDPATH)/tools/swagger/templates:$(COMPILEBUILDPATH)/tools/swagger/templates -w $(COMPILEBUILDPATH)/tools/swagger/templates ${SWAGGER_IMAGENAME}:${SWAGGER_VERSION}
...
MOCKERY=$(DOCKERCMD) run --rm -u $(shell id -u):$(shell id -g) -v $(COMPILEBUILDPATH):$(COMPILEBUILDPATH) -w $(COMPILEBUILDPATH) ${MOCKERY_IMAGENAME}:${MOCKERY_VERSION}
...
compile_standalone_db_migrator:
	...
	@$(DOCKERCMD) run --rm -v $(COMPILEBUILDPATH):$(GOBUILDPATHINCONTAINER) -w $(GOBUILDPATH_STANDALONE_DB_MIGRATOR) $(GOBUILDIMAGE) $(GOIMAGEBUILD_COMMON) -o $(GOBUILDPATHINCONTAINER)/$(GOBUILDMAKEPATH_STANDALONE_DB_MIGRATOR)/$(STANDALONE_DB_MIGRATOR_BINARYNAME)
	@echo "Done."
...
build:
	make -f $(MAKEFILEPATH_PHOTON)/Makefile $(BUILDTARGET) -e DEVFLAG=$(DEVFLAG) -e GOBUILDIMAGE=$(GOBUILDIMAGE) \
	 ...
	 -e NPM_REGISTRY=$(NPM_REGISTRY) -e NOTARYFLAG=true -e CHARTFLAG=true -e BASEIMAGETAG=$(BASEIMAGETAG) -e BASEIMAGENAMESPACE=$(BASEIMAGENAMESPACE) \
